...
# LoRA训练用什么底模
# https://www.bilibili.com/video/BV1hk4y1Y7sc/

# Train data path | 设置训练用模型、训练集
$pretrained_model = "./sd-models/v1-5-pruned.ckpt" # base model path | 底模路径
$train_data_dir = "./dataset/folder/doufeng2/imgs" # train dataset path | 训练数据集路径
$reg_data_dir = ""	# reg dataset path | 正则数据集化路径
$network_weights = "" # pretrained weights for LoRA network | 若需要从已有的 LoRA 模型上继续训练，请填写 LoRA 模型路径。

# Output settings | 输出设置
$output_name = "bdsqlsz" # output model name | 模型保存名称
$output_dir ="$train_data_dir/../models"
$logging_dir="$train_data_dir/../logs"
$save_model_as = "safetensors" # model save ext | 模型保存格式 ckpt, pt, safetensors
$mixed_precision = "bf16" # bf16效果更好，默认fp16
$save_precision="fp16" # bf16效果更好，默认fp16

# Train related params | 常用调整训练相关参数
$resolution = "576,576" # image resolution w,h. 图片分辨率，宽,高。支持非正方形，但必须是 64 倍数。
$batch_size = 2 # batch size 一次性训练图片批处理数量，根据显卡质量对应调高。
$max_train_epoches = 4 # max train epoches | 最大训练 epoch
...
#noise | 噪声
# 金字塔噪声，接近MJ的效果  https://www.bilibili.com/video/BV12P411y76P/
...
# Lion优化器介绍
# https://www.bilibili.com/video/BV1pA411m7WA/ 
...
# 新增优化器 Sophia(2倍速1.7倍显存)、Prodigy神童优化器，可自适应Dylora
# 神童优化器，全程自适应训练
# https://www.bilibili.com/video/BV1nX4y1H7x5/


$is_v2_model = 0 # SD2.0 model | SD2.0模型 2.0模型下 clip_skip 默认无效
$v_parameterization = 0 # parameterization | 参数化 v2 非512基础分辨率版本必须使用。

# 正则化和卷积的应用 https://www.bilibili.com/video/BV1MM41177xv/
# 正则化制作功能LoRA  https://www.bilibili.com/video/BV1oP411f7fi/


$training_comment = "this LoRA model credit from xxx" # training_comment | 训练介绍，可以写作者名或者使用触发关键词
$dataset_class = ""

# 完美炼丹术，差异炼丹法
# https://www.bilibili.com/video/BV11m4y147WQ/
...
$algo = "locon" # algo参数，指定训练lycoris模型种类，包括lora(就是locon)、loha、IA3以及lokr、dylora ，5个可选
# 哈达玛积训练画风LoRA
# https://www.bilibili.com/video/BV1HM4y167Hg/
# 动态lora！任意dim值选择
# https://www.bilibili.com/video/BV1Qc411n7V8/
...
# block weights | 分层训练
# LORA分层训练的底层原理
# https://www.bilibili.com/video/BV1th411F7CR/
...
$enable_enable_block_dim=0 #开启dim分层训练
...
$sample_prompts="./toml/vector.txt" #prompt文件路径
...
# 一边炼丹一边跑图！玩转wandb，最新优化器算法以及训练脚本更新
# https://www.bilibili.com/video/BV1As4y1R7cu/
...
if($optimizer_type -ilike "DAdapt*"){
  ...
  $lr = "1"
  if($optimizer_type -ine "DAdaptLion"){
      $lr = "0.33"
  }
...
if($optimizer_type -ieq "Lion" -or $optimizer_type -ieq "Lion8bit"){
	[void]$ext_args.Add("--optimizer_type=$optimizer_type")
	[void]$ext_args.Add("--optimizer_args")
  [void]$ext_args.Add("weight_decay=0.01")
...
elseif ($train_text_encoder_only) {
...
accelerate launch --num_cpu_threads_per_process=8 "./train_network.py" `
  ...
  --tokenizer_cache_dir="./huggingface" `
  --xformers $ext_args
